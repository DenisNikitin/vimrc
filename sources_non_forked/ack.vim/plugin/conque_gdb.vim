...
if g:conque_gdb_gdb_py_support
    if !exists('g:ConqueGdb_ToggleBreak')
        let g:ConqueGdb_ToggleBreak = '<F4>'
    endif
" else
...
if !exists('g:ConqueGdb_Run')
    let g:ConqueGdb_Run = '<S-F8>'
endif
if !exists('g:ConqueGdb_Next')
    let g:ConqueGdb_Next = '<F6>'
endif
if !exists('g:ConqueGdb_Step')
    let g:ConqueGdb_Step = '<F5>'
endif
if !exists('g:ConqueGdb_Print')
    let g:ConqueGdb_Print = '<F2>'
endif
if !exists('g:ConqueGdb_Finish')
    let g:ConqueGdb_Finish = '<C-F6>'
endif
if !exists('g:ConqueGdb_Backtrace')
    let g:ConqueGdb_Backtrace = '<F3>'
endif
if !exists('g:ConqueGdb_Kill')
    let g:ConqueGdb_Kill = '<C-F8>'
endif
if !exists('g:ConqueGdb_SaveBP')
    let g:ConqueGdb_SaveBP = '<S-F7>'
endif
if !exists('g:ConqueGdb_RestoreBP')
    let g:ConqueGdb_RestoreBP = '<C-F7>'
endif
...
exe 'nnoremap <silent> ' . g:ConqueGdb_Backtrace . ' :call conque_gdb#command("backtrace full")<CR>'
exe 'nnoremap <silent> ' . g:ConqueGdb_Kill . ' :call conque_gdb#command("kill")<CR>y<CR>'
exe 'nnoremap <silent> ' . g:ConqueGdb_SaveBP . ' :call conque_gdb#command("save breakpoints ./brks_" . expand("%<"))<CR>'
exe 'nnoremap <silent> ' . g:ConqueGdb_RestoreBP . ' :call conque_gdb#command("source ./brks_" . expand("%<"))<CR>'
